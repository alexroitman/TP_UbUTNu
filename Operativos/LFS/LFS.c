...
void receptorDeSockets(int* socket) {
	...
	while (1) {
		...
		case DESCRIBE:
			...
			log_debug(logger, "me llego la tabla %s",packDescribe->nombre_tabla);
...
while (1) {
	...
	case CREATE:
		...
		errorHandler = Create(packCreateConsola->nombre_tabla,
				packCreateConsola->consistencia, packCreateConsola->particiones,
...
t_tabla* tabla_encontrada = (t_tabla*) list_find(memtable, (void*) &es_tabla);
if (tabla_encontrada)
	return 1;
return 0;
}
...
string_append_with_format(&tabla, "%sTablas/%s/metadata", configLFS->dirMontaje,
		nombre_tabla);
